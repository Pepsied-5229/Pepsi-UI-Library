--if shared.unloadall then shared.unloadall() end loadstring(workspace.Scripts.Ui.PepsiLib.Source)() -- again, for studio command bar (auto cleanup for fast changes)
-- Ignore these comments, it's just a very VERY cheap studio compatibility api. its bad, but works for this purpose----------------------------------------------------------------------------
--local Library, Flags, Subs = (function() -- simulating an artificial require()

local pcall = pcall
local __opcall = pcall
if game:GetService("RunService"):IsStudio() then
	getgenv = getfenv -- using my elite haxor skills to port getgenv into studio >:)
	__files = {}
	function makefolder()
	end
	function isfolder(s)
		...
		local succezz = pcall(function()
			...
			if (typ == "number") and (image > 0) then
				pcall(function()
					...
					elseif nfo.AssetTypeId == 13 then -- ughhh..... Robust the fuck out of one damn feature
...
local Color3FromHex do
	local pepsi_color = Color3.fromRGB(139, 51, 20)
	local ResolveHex = Color3.fromHex
	if ResolveHex and (pcall(ResolveHex, "8b3314") == pepsi_color) then
		ResolveHex = Color3.fromHex
	else
	...
	do -- prompts
		...
		function library.Prompt(self, PromptData, ...)
			...
			do
				...
				Buttons.ScrollBarImageTransparency = 1.000
...
function library:CreateWindow(options, ...)
	...
	do -- show Hide KBBBBBBBBbbbbbbbb
		local os_clock = os.clock
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local function savestuff(s, get)
						...
						local saving = {}
						for cflag, data in next, working_with do
